...
def compose(*, background: str, layout: str, tone: str, title: str,
    ...
    if plate:
        ...
        if used:
            # ★ **「비워라」를 「한가하게 두라」로 적는다.** 처음엔 "바탕이 넓게
            #   드러나게 둬라" 라고 썼더니 모델이 네 귀퉁이를 통째로 비우고
            #   장면을 가운데 띠로 몰았다 — 화면이 헐거워졌다(2026-09-18 실측,
            #   1장 002·003). 바로 위에 있는 "네 가장자리까지 다 써라" 와
            #   정면으로 부딪혔고 이 줄이 이겼다.
            #   비우라는 것은 **빈 공간**이 아니라 **한가한 자리**다 — 장면은
            #   가장자리까지 차 있고, 그 자리만 잔무늬 없이 고르면 된다.
            lines.append(
                "라벨 자리(글자가 얹힐 곳): " + " · ".join(f"「{x}」" for x in used) +
                f" — 모두 {len(used)}곳이다. **화면은 네 가장자리까지 꽉 채운 채로**, "
                "이 자리들만 잔무늬·작은 사물·복잡한 결이 없는 **한가한 면**이 "
                "되게 하라 — 큰 면, 고른 벽, 넓은 바닥, 열린 하늘 같은 것이다. "
                "비우라는 말이 아니다. 장면을 가운데로 몰거나 가장자리를 비우면 "
                "화면이 헐거워진다. 화면이 이 자리에 글자를 얹으므로 "
                "**글자는 네가 그리지 않는다**")
        # 라벨(콜아웃)만 뺀다.
        lines.append(
