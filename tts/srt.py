...
def split_into_cues(text: str, max_chars: int = _CUE_MAX_CHARS) -> list[str]:
    ...
    표현 태그(<laugh> 등)는 먼저 제거한다. 태그는 모델에게 주는 연출 지시일
    뿐이라 화면에 "<laugh>"가 찍히면 안 된다. 길이 계산에서도 빠져야 자막
    타이밍이 실제 낭독 분량에 비례한다.
    """
    text = tags.strip(text or "").strip()
    if not text:
        return []
    cues: list[str] = []
...
def merge_scene_cues(scenes: list[tuple[list[Cue], float]],
    ...
    ★ crossfade — 씬을 이어붙일 때 경계마다 겹치는 초.
      ...
      기본값 0.0 은 crossfade 없이 그냥 이어 붙이는 경우(supertonic_tts 단독 사용)다.
