(() => {
  // 사이드바(shell.js)와의 접점. shell.js가 window.__s3에 채워 넣는다.
  // app.js가 먼저 로드되므로 호출 시점에 조회한다.
  const currentLang = () => (window.__s3 && window.__s3.lang && window.__s3.lang()) || 'ko';
  const showMetrics = (h) => { if (window.__s3 && window.__s3.showMetrics) window.__s3.showMetrics(h); };
  ...
  // 표현 태그는 모델에게 주는 연출 지시라 자막에 찍히면 안 된다.
  // 한 개 합성 탭은 SRT를 서버가 아니라 브라우저에서 만들기 때문에, 여기서도
  // backend tags.strip() 과 같은 규칙으로 지워줘야 한다.
  const EXPRESSION_TAG_RE =
    /<(laugh|breath|sigh|surprise|scream|throatclear|sad|angry|cough|yawn)>/gi;
  function stripTags(text) {
    return (text || '')
      .replace(EXPRESSION_TAG_RE, ' ')
      .replace(/\s+([,.!?;:…])/g, '$1')   // 태그를 지우며 생긴 ' .' 자국 정리
      .replace(/[ \t]{2,}/g, ' ')
      .trim();
  }
  ...
  function splitIntoCues(text, maxChars = 55) {
    text = stripTags(text);   // 길이 계산에서도 빠져야 타이밍이 맞다
    if (!text) return [];
  ...
  document.getElementById('generate').addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/api/synthesize', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: pronText, voice, speed, total_step, lang: currentLang() }),
      });
      if (!res.ok) {
        let detail = res.statusText;
        try { detail = (await res.json()).detail || detail; } catch {}
