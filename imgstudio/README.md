...
## 슬라이드 JSON 일괄 생성 (80장까지 한 번에)

이미지 스튜디오 하단 **📋 → 「슬라이드 JSON」** 탭에 강의 슬라이드 프롬프트 JSON
(`{"prompts": [{"n": 3, "prompt": "..."}]}`)을 넣으면, 지정한 폴더에
**슬라이드 번호 파일명**(`003.png`)으로 한꺼번에 만들어 줍니다.

- 번호 범위·`place=true`만 골라 생성 · 파일명은 `003.png` 또는 `003_제목.png`
- 동시 생성 1~6장(기본 3) · 실패 항목 자동 재시도 · **실패 재시도** 버튼
- 이미 있는 번호는 건너뛰므로 중단했다가 다시 눌러도 **남은 장만** 생성
- 브라우저를 닫아도 서버에서 계속 진행되고, 다시 열면 진행바가 복구됩니다
- 결과는 폴더 + (옵션) 스튜디오 갤러리에 함께 담겨 바로 수정할 수 있습니다
